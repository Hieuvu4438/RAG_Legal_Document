...
with st.sidebar:
    ...
    ollama_model = st.text_input("Ollama Model", value=settings.llm_model)
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if ollama_model in model_names:
                    st.success(f"✅ Ollama connected! Model '{ollama_model}' is available.")
                else:
                    st.warning(f"⚠️ Ollama connected but model '{ollama_model}' not found. Available: {model_names}")
            else:
...
with col1:
    st.markdown('''
        ...
        <h3 style='color: #fff; margin-top: 0; margin-bottom: 16px;'>
            Search over legal corpus of 50 million+ vectors in milliseconds
        </h3>
...
if prompt := st.chat_input("Ask a question about your document..."):
    ...
    try:
        ...
        if response.status_code != 200:
            st.error("⚠️ Ollama is not running. Please start Ollama server.")
            st.stop()
    except Exception:
